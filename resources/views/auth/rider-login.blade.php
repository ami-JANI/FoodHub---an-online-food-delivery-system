...
@section('title', 'Rider Sign in - FoodHub')

@section('content')
    <h1 class="text-xl font-bold mb-1">Rider sign in</h1>
    <p class="text-sm text-gray-500 dark:text-gray-400 mb-5">Pick up nearby orders and start delivering.</p>
    ...
    <form method="POST" action="{{ route('rider.login') }}" class="space-y-3">
        @csrf
        <div>
    ...
    <p class="text-sm text-gray-500 dark:text-gray-400 mt-4 text-center">
        New rider? <a href="{{ route('rider.register') }}" class="text-rose-800 dark:text-rose-400 hover:underline">Apply now</a>
    </p>
@endsection
